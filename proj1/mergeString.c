...
void MergeSort(movie *arr, int low, int high,int Scases,char* formatFile)
{
	//printf("%s format\n\n\n",formatFile);
    ...
    if (low < high)
    {
        ...
        MergeSort(arr, low, mid,Scases,formatFile);
        MergeSort(arr, mid + 1, high,Scases,formatFile);

        Merge(arr, low, mid, high,Scases,high,formatFile);
        //printf("<<-- %s: lo = %d, md = %d, hi = %d\n", __func__, low, mid, high);
    }
}
void Merge(movie *arr, int low, int mid, int high,int cases,int lines,char* format)
{
    ...
    Print("merged", mergedSize, temp,format);
...
void Print(const char *tag, int n, movie *array,char* name_of_file)
{
	FILE* output_file=fopen(name_of_file,"w");
	if(output_file==NULL){
	
		printf("FILE NOT OPENED I AM IN PRINT NMA= %s\n",name_of_file);
	}
	...
	int z=0;
	for(z=0;z<strlen(array[i].movieTitle);z++){
		if(array[i].movieTitle[z]==','){
		fprintf(output_file," \"%s\"\t ",array[i].movieTitle);
		break;
		}
	}
	if(z==strlen(array[i].movieTitle)){
        fprintf(output_file," %s\t ",array[i].movieTitle);
	}
        fprintf(output_file," %d\t ",array[i].voted_users);
